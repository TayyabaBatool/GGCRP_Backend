...
module.exports = {
    ...
    getMarks: async (req, res, next) => {
        try {
            ...
            const Assignment = getMarks.filter((obj) => {
                return obj.exam === "Assignment"
            })
            const Mids = getMarks.filter((obj) => {
                return obj.exam === "Mids"
            })
            const Final = getMarks.filter((obj) => {
                return obj.exam === "Final"
            })
            res.status(200).json({
                result: {
                    Assignment,
