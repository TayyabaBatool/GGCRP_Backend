...
module.exports = {
    ...
    markAttendence: async (req, res, next) => {
        try {
            ...
            const DateFound = await Attendence.find({ date:Date,subjectCode:subjectCode })
            // const subCodeFound = await Attendence.find({ subjectCode })
            // console.log(DateFound[*].date)
            // console.log(subCodeFound[*].subjectCode)
            if((DateFound.date === Date) && (subCodeFound.subjectCode === subjectCode)){
                console.log("Attendence for this subject in this date is already marked") 
                res.status(400).json({ message: "already marked for that Date" })
            }
            else{
            ...
            for (let i = 0; i < filteredArr.length; i++) {
                ...
                if (!pre) {
                    ...
                    attendence.subjectCode = subjectCode
    ...
    uploadMarks: async (req, res, next) => {
        try {
            ...
            // const student = await Student.find({year,department,section})
            // console.log(student[0].name);
            // var studentName = student[0].name;
            const isAlready = await Mark.find({ exam, department,section,subjectId:subject._id, subjectCode })
